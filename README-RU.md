...
Некоторые слушатели событий включены ([список слушателей](#event-listeners)):
...
[Полный список слушателей событий.](#event-listeners)
...
**Никогда не используйте delay'и, sleep'ы и т.п.**. Вместо них используйте слушатели [Interval](#interval) или [Timeout](#timeout). Иначе слушатели будут работаь не корректно и с задержками.
...
Параметры:
* `Asynchrony::Listener* listener` Указатель на объект слушателя, который является экземпляром одного из классов, наследующего класс `Asynchrony::Listener`. [Список готовых слушателей](#event-listeners).
...
Некоторые слушатели событий можно привязать, использую упрощённые методы (полный список можно посмотреть в [списке слушателей](#event-listeners)):
...
Смотрит состояние кнопки, отфильтровывает дребезг контактов и вызывает срабатывание, если кнопка находится в нужном состоянии.
